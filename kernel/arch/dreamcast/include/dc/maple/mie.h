...
/** \defgroup mie_jvs_masks Input Masks
    ...
    For the cabinet panel, use \p MIE_JVS_PANEL_DIP*_MASK
    or \p MIE_JVS_PANEL_*_MASK with the matching \ref mie_jvs_input_t value.
...
#define MIE_JVS_PANEL_DIP1_MASK    BIT(0)  /**< \brief Panel DIP switch 1 mask. */
#define MIE_JVS_PANEL_DIP2_MASK    BIT(1)  /**< \brief Panel DIP switch 2 mask. */
#define MIE_JVS_PANEL_DIP3_MASK    BIT(2)  /**< \brief Panel DIP switch 3 mask. */
#define MIE_JVS_PANEL_DIP4_MASK    BIT(3)  /**< \brief Panel DIP switch 4 mask. */
#define MIE_JVS_PANEL_TEST_MASK    BIT(0)  /**< \brief Panel test switch (PSW1). */
#define MIE_JVS_PANEL_SERVICE_MASK BIT(1)  /**< \brief Panel service switch (PSW2). */
...
/** \brief   Set an automatic native JVS input callback.
    ...
    For \p MIE_JVS_IN_P1 and \p MIE_JVS_IN_P2, \p mask uses \ref mie_jvs_masks.
    For \p MIE_JVS_IN_SYSTEM, \p mask uses the system switch masks from
    \ref mie_jvs_masks.
    For \p MIE_JVS_IN_PANEL_DIP, \p mask uses the DIP masks from \ref mie_jvs_masks.
    For \p MIE_JVS_IN_PANEL_PSW, \p mask uses the panel service switch masks
    from \ref mie_jvs_masks.
