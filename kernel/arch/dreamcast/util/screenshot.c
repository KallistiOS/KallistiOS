...
size_t vid_screen_shot_data(uint8_t **buffer) {
    ...
    buffer_size = numpix * bpp + header_size;
    *buffer = (uint8_t *)malloc(buffer_size);
    ...
    if(*buffer == NULL) {
        dbglog(DBG_ERROR, "create_image_data: can't allocate memory\n");
        return 0;
    }

    /* Set the data pointer to the location after the header */
    ...
    switch(vid_mode->pm) {
        ...
        case PM_RGB0888:  /* (32-bit) */
            ...
            break;
        default:
            dbglog(DBG_ERROR, "create_image_data: can't process pixel mode %d\n", vid_mode->pm);
            free(*buffer);
            *buffer = NULL;
            irq_restore(save);
            return 0;
...
int vid_screen_shot(const char *destfn) {
    ...
    /* Write the data */
    if(fs_write(f, buffer, buffer_size) != (ssize_t)(buffer_size)) {
