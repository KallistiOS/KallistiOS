...
int vmu_pkg_build_ex(vmu_pkg_t *src, uint8_t **dst, int *dst_size, const void *intl_dst) {
    ...
    if(intl_dst) {
        /* Count initial data for GAME file types */
        out_size += 512;
    }
    ...
    *dst = out;
    ...
    /* Copy initial data for GAME file types */
    if(intl_dst) {
        memcpy(out, intl_dst, 512);
        out += 512;
    }
    ...
    hdr->crc = 0;
    hdr->data_len = intl_dst ? 0 : src->data_len;
...
/* Converts a vmu_pkg_t structure into an array of uint8_t's for DATA file types */
int vmu_pkg_build(vmu_pkg_t *src, uint8_t **dst, int *dst_size) {
    return vmu_pkg_build_ex(src, dst, dst_size, NULL);
}
...
int vmu_pkg_parse_ex(uint8_t *data, size_t data_size, vmu_pkg_t *pkg, bool ftgame, bool nocrc) {
    ...
    int ec_size;
    size_t hdr_size, icon_size, total_size, initial_size;
    vmu_hdr_t *hdr;

    assert(data && pkg);

    initial_size = ftgame ? 512 : 0;
    hdr = (vmu_hdr_t *)(data + initial_size);
    ...
    total_size = hdr->data_len + hdr_size + initial_size;

    if(ec_size < 0 || icon_cnt < 1 || icon_cnt > 3 || (ftgame && hdr->data_len) || total_size > data_size) {
        /* Invalid eyecatch_type, icon_cnt, data_len, or truncated buffer. */
        dbglog(DBG_ERROR, "vmu_pkg_parse_ex: truncated buffer or corrupted header\n");
        return -2;
    }
     ...
     * This method may fail with "const data[]" and "-O2" in kos-cc */
    if(ftgame) {
        /* No checksum for GAME (there no payload data) */
        crc_save = hdr->crc;
        crc = 0x0000;
    } else {
        crc_save = hdr->crc;
    ...
    if(crc_save != crc) {
        dbglog(DBG_ERROR, "vmu_pkg_parse_ex: expected CRC %04x, got %04x\n", crc, crc_save);
        if(!nocrc) {
            return -1;
        }
    }
    ...
    pkg->icon_data = data + sizeof(vmu_hdr_t) + initial_size;
    pkg->eyecatch_data = data + sizeof(vmu_hdr_t) + initial_size + icon_size;
    pkg->data = data + hdr_size + initial_size;
    ...
    *(pkg->app_id     + sizeof(hdr->app_id))     = '\0';
    ...
    if(ftgame) {
        pkg->data_len = (int)(data_size - (hdr_size + initial_size));
    }
...
/* Parse an array of uint8_t's (i.e. a VMU data file) into a
 * vmu_pkg_t package structure with header at offset 0. */
int vmu_pkg_parse(uint8_t *data, size_t data_size, vmu_pkg_t *pkg) {
    return vmu_pkg_parse_ex(data, data_size, pkg, false, false);
}
