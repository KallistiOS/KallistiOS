...
static void maple_hw_init(void) {
    ...
    assert_msg(__is_aligned((uintptr_t)maple_state.dma_buffer, 32), "DMA buffer was unaligned; bug in dlmalloc; please report!");

    /* Force it into the P2 area */
    maple_state.dma_buffer = (uint8_t *)((((uintptr_t)maple_state.dma_buffer) & MEM_AREA_CACHE_MASK) | MEM_AREA_P2_BASE);
    ...
    dbglog(DBG_SOURCE(MAPLE_DMA_DEBUG), "  DMA Buffer at %0x" PRIxPTR "\n", (uintptr_t)maple_state.dma_buffer);
...
void maple_hw_shutdown(void) {
    ...
    if(maple_state.dma_buffer != NULL) {
        uintptr_t ptr = (uintptr_t)maple_state.dma_buffer;
